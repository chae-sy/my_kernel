...
__launch_bounds__(16 * 16)
__global__ void mma_matmul_2_0(const half *A, const half *B, half *C, int M, int N, int K) {
  ...
  for (int m = 0; m < 2; m++) {
    for (int n = 0; n < 2; n++) {
      int mTile = m * 16;
      int nTile = n * 8;
      half2 d0 = __floats2half2_rn(dReg[m][n][0], dReg[m][n][1]);
      half2 d2 = __floats2half2_rn(dReg[m][n][2], dReg[m][n][3]);

      half2 *cOut0 = reinterpret_cast<half2 *>(
          &C[(mBlock + mTile + 2 * mWarp + groupID) * N +
             nBlock + nTile + 2 * nWarp + 2 * groupLaneID]);

      half2 *cOut2 = reinterpret_cast<half2 *>(
          &C[(mBlock + mTile + 2 * mWarp + groupID + 8) * N +
             nBlock + nTile + 2 * nWarp + 2 * groupLaneID]);
...
__launch_bounds__(16 * 16)
__global__ void mma_matmul_2_1(const half *A, const half *B, half *C, int M, int N, int K) {
  __shared__ uint4 As[32][8];
  __shared__ uint4 Bs[32][8];
  ...
  for (int m = 0; m < 2; m++) {
    for (int n = 0; n < 2; n++) {
      ...
      half2 *cOut0 = reinterpret_cast<half2 *>(
          &C[(blockRowStart + mTile + 2 * warpOffsetA + groupID) * N +
             blockColStart + nTile + 2 * warpOffsetB + 2 * groupLaneID]);
      half2 *cOut2 = reinterpret_cast<half2 *>(
          &C[(blockRowStart + mTile + 2 * warpOffsetA + groupID + 8) * N +
             blockColStart + nTile + 2 * warpOffsetB + 2 * groupLaneID]);
